...
GBufferedImage* GBufferedImage::diff(GBufferedImage& image, int diffPixelColor) const {
    ...
    result->Fill(0, 0, w1, h1, m_backgroundColor);
    for (int y = 0; y < hmin; y++) {
        for (int x = 0; x < wmin; x++) {
            int px1 = m_pixels[y][x];
            int px2 = image.m_pixels[y][x];
...
void GBufferedImage::Fill(double x, double y, double width, double height, int rgb) {
    checkIndex("Fill", x, y);
    checkIndex("Fill", x + width - 1, y + height - 1);
    checkColor("Fill", rgb);
    ...
    getPlatform()->gbufferedimage_Fill(this, x, y, width, height, rgb);
}

void GBufferedImage::Fill(double x, double y, double width, double height, std::string rgb) {
    Fill(x, y, width, height, convertColorToRGB(rgb));
}
